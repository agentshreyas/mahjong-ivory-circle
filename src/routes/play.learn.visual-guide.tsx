...
function Melds() {
  ...
  return <><div className="flex gap-2 overflow-x-auto pb-2">{melds.map((item, index) => <Button key={item.name} type="button" size="sm" variant={selected === index ? "default" : "outline"} onClick={() => setSelected(index)} className={selected === index ? "rounded-full bg-[var(--hsbc)] text-[var(--ivory)] hover:bg-[var(--hsbc-pressed)]" : "rounded-full border-[var(--hairline)] bg-white text-[var(--ink)]"}>{item.name}</Button>)}</div><div className="mt-4 border-l-2 border-[var(--hsbc)] bg-[var(--sand)]/50 p-4"><p className="text-[13px] leading-6 text-[var(--taupe)]">{meld.description}</p><div className="mt-4"><TileRow tiles={meld.tiles} /></div></div></>;
}
